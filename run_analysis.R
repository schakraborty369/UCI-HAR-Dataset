...
Training_set <- read.table("./train/X_train.txt")                 #Training set, dataframe of dim 7352 * 561#
Test_set <- read.table("./test/X_test.txt")                       #Test set, dataframe of dim 2947 * 561#
Subject_trained <- read.table("./train/subject_train.txt")[,1]    #Vector of trained subjects length 7352#
Subject_tested <- read.table("./test/subject_test.txt")[,1]       #Vector of tested subjects length 2947#
Training_labels <- read.table("./train/y_train.txt")[,1]          #Training labels vector length 7352#
Test_labels <- read.table("./test/y_test.txt")[,1]                #Test labels vector length 2947#
tempx <- rbind(Training_set, Test_set)
features <- read.table("features.txt")[,2]                        #Feature vector length 561#
...
filtered_data <- select(Combined_data, contains("Subjects"), 
                                        contains("mean"), 
                                          contains("std"), 
                                            contains("Activity"),
                                              -contains("meanFreq"),
...
#==============================================================================================================
#5. Creating a secondary independent tidy data set with the average of each variable for each activity 
#   and each subject
#==============================================================================================================
